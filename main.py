...
def shared_budget(filename = "data.csv"):
    ...
    investmen_dict = pd.DataFrame.to_dict(df.groupby(by = "Buyer ").agg("sum"))["Amount"]
    summary_dict = {key: investmen_dict[key] - consume_dict.get(key, 0) for key in investmen_dict}
    return summary_dict
